...
description: "At Medplum, we've enabled the Developer Certificate of Origin (DCO) for all contributions to our repositories. This means every commit to the Medplum codebase needs to be \"signed off\" by the author. We understand this might be a new process for some, and we're here to help you get started."
---

## What is DCO and Why Do We Use It?[​](#what-is-dco-and-why-do-we-use-it "Direct link to What is DCO and Why Do We Use It?")
...
### Why Medplum Requires DCO[​](#why-medplum-requires-dco "Direct link to Why Medplum Requires DCO")
...
## How to Sign Off Your Commits[​](#how-to-sign-off-your-commits "Direct link to How to Sign Off Your Commits")

The DCO sign-off needs to be present on *every commit* in a pull request. Here are the simplest ways to ensure your commits are signed off:

### 1. Signing Off New Commits (Recommended)[​](#1-signing-off-new-commits-recommended "Direct link to 1. Signing Off New Commits (Recommended)")
...
### 2. Fixing Existing Commits (If You Forgot)[​](#2-fixing-existing-commits-if-you-forgot "Direct link to 2. Fixing Existing Commits (If You Forgot)")

It's common to forget the `-s` flag sometimes, especially when you're just getting used to it. Don't worry, you can fix it!

#### Option A: Amending the Last Commit[​](#option-a-amending-the-last-commit "Direct link to Option A: Amending the Last Commit")
...
#### Option B: Rebasing to Sign Off Multiple Commits[​](#option-b-rebasing-to-sign-off-multiple-commits "Direct link to Option B: Rebasing to Sign Off Multiple Commits")
...
```
pick a1b2c3d Commit 1 messagepick e4f5g6h Commit 2 messagepick i7j8k9l Commit 3 message
```

Change to:

```
reword a1b2c3d Commit 1 messagereword e4f5g6h Commit 2 messagereword i7j8k9l Commit 3 message
```
...
## Tips and Tricks for Convenience[​](#tips-and-tricks-for-convenience "Direct link to Tips and Tricks for Convenience")

While there's no single `git config` option to *always* sign off every commit automatically, here are some helpful tricks:

### 1. Git Alias for Signed Commits[​](#1-git-alias-for-signed-commits "Direct link to 1. Git Alias for Signed Commits")
...
### 2. Git Hooks (Advanced)[​](#2-git-hooks-advanced "Direct link to 2. Git Hooks (Advanced)")
...
```
#!/bin/shNAME=$(git config user.name)EMAIL=$(git config user.email)if [ -z "$NAME" ] || [ -z "$EMAIL" ]; then  echo "Error: Git user.name and user.email must be configured for DCO sign-off."  exit 1fiif ! grep -qs "^Signed-off-by:" "$1"; then  printf "\nSigned-off-by: %s <%s>\n" "$NAME" "$EMAIL" >> "$1"fi
```
...
[Edit this page](https://github.com/medplum/medplum/blob/main/packages/docs/docs/contributing/dco.md)
