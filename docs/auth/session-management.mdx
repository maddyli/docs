...
description: "This guide explains how Medplum manages authentication tokens and how to implement secure token handling in your applications."
---

## Access Tokens vs Refresh Tokens[​](#access-tokens-vs-refresh-tokens "Direct link to Access Tokens vs Refresh Tokens")
...
### Requesting Refresh Tokens[​](#requesting-refresh-tokens "Direct link to Requesting Refresh Tokens")
...
```
await medplum.startLogin({  email: 'admin@example.com',  password: 'password',  scope: 'openid offline_access',});
```

```
await medplum.signInWithRedirect({  scope: 'openid offline_access',});
```

## Configuring Token Lifetimes[​](#configuring-token-lifetimes "Direct link to Configuring Token Lifetimes")

### Default Token Lifetimes[​](#default-token-lifetimes "Direct link to Default Token Lifetimes")
...
### Customizing Token Lifetimes[​](#customizing-token-lifetimes "Direct link to Customizing Token Lifetimes")
...
```
const clientApplication = {  resourceType: 'ClientApplication',  // Set access token lifetime to 2 hours (7200 seconds)  accessTokenLifetime: 7200,  // Set refresh token lifetime to 180 days  refreshTokenLifetime: 15552000,};
```
...
## Managing Token Refresh with Grace Periods[​](#managing-token-refresh-with-grace-periods "Direct link to Managing Token Refresh with Grace Periods")
...
### How Grace Periods Work[​](#how-grace-periods-work "Direct link to How Grace Periods Work")
...
### Customizing the Grace Period[​](#customizing-the-grace-period "Direct link to Customizing the Grace Period")

You can adjust the grace period when initializing the Medplum client:

```
// Initialize client with a 10-minute grace periodconst customMedplum = new MedplumClient({  refreshGracePeriod: 600000, // 10 minutes in milliseconds});// You can also check authentication status with a custom grace periodif (!customMedplum.isAuthenticated(300000)) {  // Token will expire within 5 minutes  await customMedplum.refreshIfExpired();}
```

## Best Practices for Token Management[​](#best-practices-for-token-management "Direct link to Best Practices for Token Management")
...
2. **Leverage Automatic Token Refresh**
   ...
   ```
   const bestPracticesMedplum = new MedplumClient({  refreshGracePeriod: 300000, // 5 minute grace period (default)});
   ```
...
[Edit this page](https://github.com/medplum/medplum/blob/main/packages/docs/docs/auth/session-management.mdx)
