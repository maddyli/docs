...
description: "Presigned URLs are a secure mechanism Medplum uses to serve binary content (images, videos, documents). They grant , allowing the content to be safely embedded in web applications using native HTML tags like  and  without requiring extra authentication headers."
---
...
## Generating a Signing Key[​](#generating-a-signing-key "Direct link to Generating a Signing Key")
...
The resulting file contains both the public and the private key. The following example command extracts the public key from the file named private\_key.pem.
...
## Default Signing Keys (Local Development)[​](#default-signing-keys-local-development "Direct link to Default Signing Keys (Local Development)")
...
[Edit this page](https://github.com/medplum/medplum/blob/main/packages/docs/docs/self-hosting/presigned-urls.md)
