---
title: "OpenTelemetry"
description: "This page describes Medplum's optional OpenTelemetry support, and how to integrate with AWS CloudWatch."
---

## Introduction to OpenTelemetry[​](#introduction-to-opentelemetry "Direct link to Introduction to OpenTelemetry")
...
### Key Concepts of OpenTelemetry[​](#key-concepts-of-opentelemetry "Direct link to Key Concepts of OpenTelemetry")
...
## Introduction to AWS Distro for OpenTelemetry[​](#introduction-to-aws-distro-for-opentelemetry "Direct link to Introduction to AWS Distro for OpenTelemetry")
...
## Using OpenTelemetry[​](#using-opentelemetry "Direct link to Using OpenTelemetry")

### OpenTelemetry on localhost[​](#opentelemetry-on-localhost "Direct link to OpenTelemetry on localhost")

When running on localhost, OpenTelemetry is disabled by default. To enable OpenTelemetry, follow these steps:

#### 1. Start an OpenTelemetry Collector[​](#1-start-an-opentelemetry-collector "Direct link to 1. Start an OpenTelemetry Collector")
...
#### 2. Add the OpenTelemetry config settings[​](#2-add-the-opentelemetry-config-settings "Direct link to 2. Add the OpenTelemetry config settings")
...
```
export OTLP_TRACES_ENDPOINT="http://localhost:4318/v1/traces"export OTLP_METRICS_ENDPOINT="http://localhost:4318/v1/metrics"
```

#### 3. Restart the Medplum server[​](#3-restart-the-medplum-server "Direct link to 3. Restart the Medplum server")

Ctrl+C to stop the server, then `npm run dev` to restart.

### OpenTelemetry on AWS[​](#opentelemetry-on-aws "Direct link to OpenTelemetry on AWS")
...
```
{  "name": "staging",  "region": "us-east-1",  "stackName": "MedplumStagingStack",  // ...  "environment": {    "OTLP_TRACES_ENDPOINT": "http://localhost:4318/v1/traces",    "OTLP_METRICS_ENDPOINT": "http://localhost:4318/v1/metrics"  },  // ...  "additionalContainers": [    {      "name": "aws-otel-collector",      "image": "amazon/aws-otel-collector:latest",      "command": ["--config=/etc/ecs/ecs-default-config.yaml"]    }  ]}
```
...
## OpenTelemetry Resources[​](#opentelemetry-resources "Direct link to OpenTelemetry Resources")
...
[Edit this page](https://github.com/medplum/medplum/blob/main/packages/docs/docs/self-hosting/opentelemetry.md)
