---
title: "Overview"
description: "The Medplum Agent is an application that runs inside your firewall and connects to devices over low level protocols such as HL7/MLLP, ASTM, and DICOM. These network protocols are commonly unencrypted, and therefore require an adapter to a secure transport. The Medplum Agent uses secure HTTPS WebSocket channels to stream messages between devices and a Medplum server cluster in the cloud."
---
...
![Medplum Agent Overview](/images/docs/assets/images/medplum-agent-overview-2994cd81d665a4ad25aba07107401df3.webp)

The Medplum Agent securely connects to Medplum servers running in the cloud using HTTPS and WebSockets:

![Medplum Agent Connection Diagram](/images/docs/assets/images/medplum-agent-connection-diagram-33fd78c93c6f8bd9e00346db7acedc55.webp)
...
### Configuration on Medplum App[​](#configuration-on-medplum-app "Direct link to Configuration on Medplum App")
...
#### Set up an Endpoint Resource[​](#set-up-an-endpoint-resource "Direct link to Set up an Endpoint Resource")
...
```
{  "resourceType": "Endpoint",  "status": "active",  "connectionType": {    "system": "http://terminology.hl7.org/CodeSystem/endpoint-connection-type",    "code": "hl7v2-mllp",    "display": "HL7 v2 MLLP"  },  "name": "Test Endpoint",  "payloadType": [    {      "coding": [        {          "system": "http://terminology.hl7.org/CodeSystem/endpoint-payload-type",          "code": "any",          "display": "Any"        }      ]    }  ],  "address": "mllp://0.0.0.0:56000"}
```

#### Set up a Bot Resource[​](#set-up-a-bot-resource "Direct link to Set up a Bot Resource")
...
```
import { BotEvent, Hl7Message, MedplumClient } from '@medplum/core';export async function handler(medplum: MedplumClient, event: BotEvent<Hl7Message>): Promise<Hl7Message> {  return event.input.buildAck();}
```

#### Set up an Agent Resource[​](#set-up-an-agent-resource "Direct link to Set up an Agent Resource")
...
```
{  "resourceType": "Agent",  "name": "ACME Hospital Admissions",  "status": "active",  "channel": [    {      "name": "ADT Channel",      "endpoint": {        "reference": "Endpoint/my-endpoint-uuid",        "display": "Test Endpoint"      },      "targetReference": {        "reference": "Bot/my-bot-uuid",        "display": "HL7 ADT Processing Bot"      }    }  ]}
```

#### Set up a Client Application[​](#set-up-a-client-application "Direct link to Set up a Client Application")
...
* Base Url - The base URL of your Medplum server, or [https://api.medplum.com](https://api.medplum.com) for hosted Medplum
...
### Install on Host[​](#install-on-host "Direct link to Install on Host")
...
#### Running from Source via CLI[​](#running-from-source-via-cli "Direct link to Running from Source via CLI")
...
```
npm cicd packages/agentnpm run agent <base_url> <client_id> <client_secret> <agent_id>
```

#### Windows Install[​](#windows-install "Direct link to Windows Install")
...
#### Linux Install[​](#linux-install "Direct link to Linux Install")
...
### Testing a site install[​](#testing-a-site-install "Direct link to Testing a site install")
...
### Testing HL7 with Medplum CLI[​](#testing-hl7-with-medplum-cli "Direct link to Testing HL7 with Medplum CLI")
...
### Testing HL7 with Sim Hospital[​](#testing-hl7-with-sim-hospital "Direct link to Testing HL7 with Sim Hospital")
...
## Audit Events and Logging[​](#audit-events-and-logging "Direct link to Audit Events and Logging")
...
## Preparing for Scale[​](#preparing-for-scale "Direct link to Preparing for Scale")
...
## Supported Protocols[​](#supported-protocols "Direct link to Supported Protocols")
...
[Edit this page](https://github.com/medplum/medplum/blob/main/packages/docs/docs/agent/index.md)
