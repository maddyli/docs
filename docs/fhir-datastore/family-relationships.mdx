...
## This guide will show you[​](#this-guide-will-show-you "Direct link to This guide will show you")
...
| [**RelatedPerson**](/docs/api/fhir/resources/relatedperson) | This models the *relationship* between two people. For simple use cases, you can store demographic and contact information directly in the [RelatedPerson](/docs/api/fhir/resources/relatedperson) resource itself (see below). |
...
![flowchart](/images/docs/fhir-datastore/assets/images/family-relationships-flowchart-ad7e0f3563aaa4285c29af1c67198000.svg)

### Do family members participate in clinical or financial activities?[​](#family-member-participation "Direct link to Do family members participate in clinical or financial activities?")
...
Often times, family members only serve as points of contact but *do not* participate in clinical and financial activities. In those situations, [Approach #1](#approach-1) provides the simplest modeling approach that avoids using many different resource types.

### How often will multiple Patients (e.g. siblings) share family members?[​](#are-family-members-shared "Direct link to How often will multiple Patients (e.g. siblings) share family members?")
...
### Is the subject of the treatment an individual patient, or a family unit?[​](#family-unit-vs-individual-patient "Direct link to Is the subject of the treatment an individual patient, or a family unit?")
...
### Approach #1: [Patient](/docs/api/fhir/resources/patient) with Contact Information[​](#approach-1 "Direct link to approach-1")
...
| ------------------------------------------------------ | ----------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------- |
| * Simplest data model
* Single resource type to manage | - Duplication of data for shared family members | * Family members **do not** participate in clinical or billing activities
* Patients share family members infrequently | - At-home lab testing
- Adult mental health therapy |

### Approach #2: [RelatedPerson](/docs/api/fhir/resources/relatedperson) ➡ [Patient](/docs/api/fhir/resources/patient)[​](#approach-2 "Direct link to approach-2")
...
* Track the participation of family members in clinical activities | - Requires maintaining links between [RelatedPersons](/docs/api/fhir/resources/relatedperson) and [Patients](/docs/api/fhir/resources/patient)
- Duplication of data for shared family members | * Family members **do** participate in clinical or billing activities
* [Patients](/docs/api/fhir/resources/patient) share family members infrequently | - Insurance coverage where the *beneficiary* ([Patient](/docs/api/fhir/resources/patient)) is not the insurance *subscriber* ([RelatedPerson](/docs/api/fhir/resources/relatedperson))
- Parents acting on behalf of minors |

### Approach #3: [Patient](/docs/api/fhir/resources/patient) ➡ [RelatedPerson](/docs/api/fhir/resources/relatedperson) ➡ [Patient](/docs/api/fhir/resources/patient)[​](#approach-3 "Direct link to approach-3")
...
In this model, the [RelatedPerson](/docs/api/fhir/resources/relatedperson) does not store specific information about the father. It only stores the *relationship* between the father and child.
...
* Track the participation of family members in clinical activities | - Adds another layer of indirection compared to [Approach #2](#approach-2) | * Family members **do** participate in clinical or billing activities
* [Patients](/docs/api/fhir/resources/patient) often share family members | - Pediatric Care
- Neonatal Care
- Family Medicine
- Parents acting on behalf of minors |

### Approach #4: [Group](/docs/api/fhir/resources/group) of Patients[​](#approach-4 "Direct link to approach-4")
...
* Can be used to model abstract groups | - Doesn't model specific relationships between individuals. Need to supplement with Approaches [#2](#approach-2) or [#3](#approach-3) | * Focus of clinical, billing, or analytic activities is the family unit / group. | - Family Counseling
- Training & Education
- Group Therapy
- Population Health
- Veterinary Care |
...
[Edit this page](https://github.com/medplum/medplum/blob/main/packages/docs/docs/fhir-datastore/family-relationships/family-relationships.mdx)
