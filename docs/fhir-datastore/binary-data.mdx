---
title: "Binary Data"
description: "Managing, storing, and retrieving binary files is a fundamental requirement in modern healthcare applications. Medplum, an open-source, API-first healthcare technology platform, provides a structured approach to this. This article dives into how to upload binary files to Medplum using the FHIR  resource type, detailing the process and Medplum's specific optimizations."
---

## Introduction to FHIR `Binary`[​](#introduction-to-fhir-binary "Direct link to introduction-to-fhir-binary")
...
## Creating a FHIR `Binary`[​](#creating-a-fhir-binary "Direct link to creating-a-fhir-binary")

To upload binary files to Medplum, you'll first need to create a `Binary` resource.

### Using raw HTTP via `curl`[​](#using-raw-http-via-curl "Direct link to using-raw-http-via-curl")

The HTTP API to upload a `Binary` is straightforward:

```
curl -X POST \     -H "Authorization: Bearer YOUR_TOKEN" \     -H "Content-Type: image/jpeg" \     --data-binary "@./yourfile.jpg" \     https://api.medplum.com/fhir/R4/Binary
```

This is a short snippet, but each line has significant meaning.

First, the `Authorization` header is required to authenticate the request. See the [**Client Credentials tutorial**](/docs/auth/client-credentials) guide for how to obtain an access token.
...
### Using the Medplum SDK `MedplumClient`[​](#using-the-medplum-sdk-medplumclient "Direct link to using-the-medplum-sdk-medplumclient")

Medplum provides a JavaScript/TypeScript SDK that makes it easy to upload binary files.

```
const medplum = new MedplumClient({  clientId: 'YOUR_CLIENT_ID',  clientSecret: 'YOUR_CLIENT_SECRET',});const binary = await medplum.createBinary({  data: myFile,  filename: 'test.jpg',  contentType: 'image/jpeg',});console.log(binary.id);
```
...
### Create `Binary` via external URL[​](#create-binary-via-external-url "Direct link to create-binary-via-external-url")
...
## Referencing a `Binary` in an Attachment[​](#referencing-a-binary-in-an-attachment "Direct link to referencing-a-binary-in-an-attachment")
...
```
const photo = await medplum.createAttachment({  data: myFile,  filename: 'test.jpg',  contentType: 'image/jpeg',});const patient = await medplum.createResource({  resourceType: 'Patient',  photo: [photo],});
```

**b. Message Attachment - `Communication.payload.contentAttachment`:**

```
const document = await medplum.createAttachment({  data: myFile,  filename: 'test.pdf',  contentType: 'application/pdf',});const communication = await medplum.createResource({  resourceType: 'Communication',  status: 'completed',  payload: [{ contentAttachment: document }],});
```

<Warning>
  When creating an [`Attachment`](/docs/api/fhir/datatypes/attachment), we recommend that you *do not* use the `data` field to provide the full binary data string. This can make the attachment very large and can cause issues when making HTTP requests. See the [Handling External Files guide](/docs/charting/external-documents#example-creating-a-documentreference-for-a-binary-file) for an example of how to upload and reference [`Binary`](/docs/api/fhir/resources/binary) resources.
</Warning>

## Consuming a FHIR `Binary` in an Application[​](#consuming-a-fhir-binary-in-an-application "Direct link to consuming-a-fhir-binary-in-an-application")
...
```
{  "resourceType": "Patient",  "photo": [    {      "contentType": "image/jpeg",      "url": "Binary/12345"    }  ]}
```
...
```
{  "resourceType": "Patient",  "photo": [    {      "contentType": "image/jpeg",      "url": "https://storage.medplum.com/12345?token=..."    }  ]}
```
...
<Accordion title="Example: Download a Binary">
  ```
  const patient: Patient = {  resourceType: 'Patient',  photo: [    {      contentType: 'image/jpeg',      url: 'https://storage.medplum.com/binary/12345',    },  ],};// A function to return the binary idconst binaryUrl = getBinaryId(patient.photo?.[0].url);// Download the binaryawait medplum.download(`Binary/${binaryUrl}`);
  ```
</Accordion>
...
## Deep Dive on Storage URLs[​](#deep-dive-on-storage-urls "Direct link to Deep Dive on Storage URLs")
...
### Write Time[​](#write-time "Direct link to Write Time")
...
### Read Time[​](#read-time "Direct link to Read Time")
...
## Proxy `Binary` content through Medplum server[​](#proxy-binary-content-through-medplum-server "Direct link to proxy-binary-content-through-medplum-server")
...
2. **`storageBaseUrl`**: This setting tells Medplum the base URL for serving binary content. For server proxying, this should be your Medplum `baseUrl` followed by `/storage/`.
   ...
   * **Example:** If your `baseUrl` is `https://api.example.com/`, then `storageBaseUrl` should be `https://api.example.com/storage/`.

Here's an example of how these settings might appear in your Medplum server configuration (e.g., `medplum.config.json`):

```
{  "baseUrl": "https://api.example.com/",  "binaryStorage": "s3:my-medplum-binary-bucket",  "storageBaseUrl": "https://api.example.com/storage/"  // ... other configuration settings}
```
...
[Edit this page](https://github.com/medplum/medplum/blob/main/packages/docs/docs/fhir-datastore/binary-data.mdx)
