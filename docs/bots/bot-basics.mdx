---
title: "Bot Basics"
description: "Bots are an advanced Medplum feature that enable complex workflows. A  is a snippet of JavaScript code that can run on any resource change (create or update). This JavaScript code has access to a  , which itself can invoke FHIR operations."
---

**Medplum Bots** are run as [AWS Lambdas](https://aws.amazon.com/lambda/) and in heavily sandboxed environments. You can apply an [AccessPolicy](/docs/access/access-policies) to the Bot if you want to further reduce the data it can read and write.

Bots are disabled by default for accounts. Contact [info@medplum.com](mailto:info@medplum.com) if you'd like to learn more.
...
## Creating a Bot[​](#creating-a-bot "Direct link to Creating a Bot")

<Warning>
  Bots are disabled by default. Contact your [info@medplum.com](mailto:info@medplum.com) if you'd like to learn more.
</Warning>
...
![Create a Bot](/images/docs/bots/assets/images/create_bot-874b27e1ea8e36c57fde1dd9c34b57e6.png)
...
![Enter Bot Properties](/images/docs/bots/assets/images/enter_bot_properties-cced67f9cc513d6c17e21b4de93e5446.png)

Click "Create Bot" to save the Bot, and you will see an acknowledgement that the Bot has been created.

![Bot Created Acknowledgement](/images/docs/bots/assets/images/bot_created_acknowledgement-d58681273dbe361683faa50fb17bf27d.png)

## Editing a Bot[​](#editing-a-bot "Direct link to Editing a Bot")
...
![Bot Editor](/images/docs/bots/assets/images/BotResourcePage-bbc52042aa91c8543c51ea8138696b08.gif)
...
```
import { BotEvent, MedplumClient } from '@medplum/core';export async function handler(medplum: MedplumClient, event: BotEvent): Promise<any> {  // Your code here}
```
...
```
import { BotEvent, MedplumClient } from '@medplum/core';import { Patient } from '@medplum/fhirtypes';export async function handler(medplum: MedplumClient, event: BotEvent): Promise<any> {  const patient = event.input as Patient;  const firstName = patient.name?.[0]?.given?.[0];  const lastName = patient.name?.[0]?.family;  console.log(`Hello ${firstName} ${lastName}!`);  return true;}
```

When you are done editing, click "Save" to save your Bot code to Medplum.

### Understanding the code[​](#understanding-the-code "Direct link to Understanding the code")

Let's break this example down.

```
import { Patient } from '@medplum/fhirtypes';//....const patient = event.input as Patient;
```
...
```
const firstName = patient.name?.[0]?.given?.[0];const lastName = patient.name?.[0]?.family;
```
...
## Deploying a Bot[​](#deploying-a-bot "Direct link to Deploying a Bot")

Clicking "Save" in the **Editor** tab persists your Bot code to the Medplum database, but *doesn't* deploy it to run in production. To deploy your bot, click the "Deploy" button.

![Deploy Button](/images/docs/bots/assets/images/deploy_button-ea7487171f2c4657b7f9b77ea0f5c3e5.png)
...
### Creating and Deploying Bot from your IDE instead of Editor tab[​](#creating-and-deploying-bot-from-your-ide-instead-of-editor-tab "Direct link to Creating and Deploying Bot from your IDE instead of Editor tab")
...
```
{  "name": "sample-account-setup",  "id": "<BOT_ID>",  "source": "src/examples/sample-account-setup.ts",  "dist": "dist/sample-account-setup.js"}
```

Then, you can [deploy your bot from command line](/docs/bots/bots-in-production#deploying-your-bot)

## Executing a Bot[​](#executing-a-bot "Direct link to Executing a Bot")

Once your bot has been [saved](#editing-a-bot) and [deployed](#deploying-a-bot), it is time to execute the bot. There are a few different ways a bot can be executed:

1. Clicking the "Execute" button in the **Bot Code Editor**
2. Sending a POST request to the [`$execute` endpoint](/docs/api/fhir/operations/bot-execute)
3. Setting up a [Subscription](/docs/fhir-basics#listening-for-changes-subscriptions) to execute the Bot automatically based on changes (see next section).

### *Executing from the Code Editor*[​](#executing-from-the-code-editor "Direct link to executing-from-the-code-editor")
...
![Execute from Editor](/images/docs/bots/assets/images/execute_from_editor-a6be137df73af57dd24a62e0e64514bb.png)

In the output panel at the bottom right you will see the string "true", indicating that the execution was a success. To see the results of the execution, navigate to the **Event** tab. (You may need to reload the page.) The **Outcome Desc** column will show the result of the console.log.

### *Using the `$execute` endpoint*[​](#using-the-execute-endpoint "Direct link to using-the-execute-endpoint")
...
```
curl -X POST 'https://api.medplum.com/fhir/R4/Bot/<BOT_ID>/$execute' \  --header 'Content-Type: <CONTENT_TYPE>' \  --header 'Authorization: Bearer <ACCESS_TOKEN>' \  --data '<INPUT_DATA>'
```

Let's walk through each of the parameters here in more detail.

#### `BOT_ID`[​](#bot_id "Direct link to bot_id")
...
![Find your Bot ID](/images/docs/bots/img/app/bots/find_bot_id.png "Find your Bot ID")

#### `CONTENT_TYPE`[​](#content_type "Direct link to content_type")
...
#### `ACCESS_TOKEN`[​](#access_token "Direct link to access_token")
...
#### `INPUT_DATA`[​](#input_data "Direct link to input_data")
...
### *Executing automatically using a [`Subscription`](/docs/fhir-basics#listening-for-changes-subscriptions)*[​](#executing-automatically-using-a-subscription "Direct link to executing-automatically-using-a-subscription")
...
![New Subscription](/images/docs/bots/assets/images/new_subscription-07822689f5b3b57a4e984e68e78d3fef.png)

To ensure the Subscription is running, change "Status" to `Active`

![Activate Subscription](/images/docs/bots/assets/images/subscription_active-9ec2d07f0babd909562c2007f5ca8296.png)

Specify which Resources will trigger this subscription using a FHIR search string. If you're not familiar with FHIR search semantics, check out [this tutorial](/docs/search/basic-search) for a primer.

For this example, we will trigger the Subscription after a change to *any* `Patient` resource.

Change "Criteria" field to `Patient`

![Subscription Criteria](/images/docs/bots/assets/images/subscription_criteria-56ee6a3a4705a87f845c2ef24ce888c9.png)
...
![Subscription Channel](/images/docs/bots/assets/images/subscription_channel-f6d04f9422a4f9e850fc7be9c439a041.png)

Change "Payload" to `application/fhir+json`. This is similar to the [CONTENT\_TYPE](#content_type) field used by the `$execute` endpoint.

![Subscription Payload](/images/docs/bots/assets/images/subscription_payload-10a0b72c8bce343899fa32ae637141c5.png)
...
![Patient Sidebar](/images/docs/bots/img/app/bots/patient_sidebar.png)

Next, click on the "New\..." [button](https://app.medplum.com/Patient/new) ![New Patient Button](/images/docs/bots/assets/images/new_patient_button-3c0e4e238b8151dc99a0895b9f1565d9.png)

Enter a sample name such as given "Jane" family "Doe". Then Scroll down and click "OK"

![Create new Patient](/images/docs/bots/assets/images/NewPatient-8a9512fdf2b6407082eb191f6df3789c.gif)
...
![Audit Event](/images/docs/bots/img/app/bots/auditevent.png)
...
![All Audit Event](/images/docs/bots/assets/images/all_audit_events-62c5a0f49b2f76d66df80641fea4162e.png)

## Software Development Lifecycle[​](#software-development-lifecycle "Direct link to Software Development Lifecycle")

Bots written using the web editor are a great way to get started. If you would like to develop locally, test and deploy apps as part of your software development lifecycle, you refer to our next tutorial on [deploying Bots in production](/docs/bots/bots-in-production)

## Medplum Lambda Bots Size Limits[​](#medplum-lambda-bots-size-limits "Direct link to Medplum Lambda Bots Size Limits")
...
[Edit this page](https://github.com/medplum/medplum/blob/main/packages/docs/docs/bots/bot-basics.md)
