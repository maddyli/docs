---
title: "Rate Limits"
description: "The Medplum API uses a number of safeguards against bursts of incoming traffic to help maximize its stability. Users who send many requests in quick succession may receive HTTP error responses with status code ."
---

## Total Requests Rate Limit[​](#total-requests-rate-limit "Direct link to Total Requests Rate Limit")
...
## FHIR Interaction Load Rate Limit[​](#fhir-interaction-load-rate-limit "Direct link to FHIR Interaction Load Rate Limit")
...
FHIR uses [specific terminology](http://hl7.org/fhir/restful-interaction) to categorize different interactions with the data store, e.g. `search` and `update`. These interactions are weighted by complexity and impact to the data store, with the sum of each user's interactions in a given minute compared to the configured limit. There is also a limit on the total interactions for all users within a Project as a whole, which defaults to ten times the per-user limit.

### How to view and configure FHIR quota rate limits[​](#how-to-view-and-configure-fhir-quota-rate-limits "Direct link to How to view and configure FHIR quota rate limits")
...
* `totalFhirQuota`: Sets a limit for the *sum* of all users in the project.
...
## Reporting Request and Load Rate Limits: HTTP Headers[​](#reporting-request-and-load-rate-limits-http-headers "Direct link to Reporting Request and Load Rate Limits: HTTP Headers")
...
Each logical limit reports the number of units remaining (`r`) and the time remaining, in seconds, until the limit resets (`t`). Applications should use these reported values to proactively slow down activity when few units are remaining, in order to prevent service disruption. Once a `429 Too Many Requests` error is produced, applications should not retry until after at least the indicated number of seconds have passed and the limit has been reset.

[Edit this page](https://github.com/medplum/medplum/blob/main/packages/docs/docs/rate-limits/index.md)
