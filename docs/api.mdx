---
title: "Overview"
description: "Welcome to the Medplum API reference. This section is intended as reference material, overview and how-to guides can be found in the  section. The base URL for hosted Medplum is ."
---
...
* Store the client\_id and client\_secret for authentication
* Process JSON response files

## Authentication and Authorization[​](#authentication-and-authorization "Direct link to Authentication and Authorization")

If you are using Medplum as an identity-provider, [**OAuth**](/docs/api/oauth) documents the endpoints supplied by the Medplum server to complete the [OAuth2 Auth Code Flow](https://auth0.com/docs/get-started/authentication-and-authorization-flow/authorization-code-flow). Medplum supports a wide variety of authentication and authorization options, including SMART-on-FHIR scopes.

### SMART-on-FHIR[​](#smart-on-fhir "Direct link to SMART-on-FHIR")

Medplum uses the SMART Application Launch Framework Implementation Guide ([http://hl7.org/fhir/smart-app-launch/1.0.0/](http://hl7.org/fhir/smart-app-launch/1.0.0/)) and provides an OAuth 2.0 compliant authorization server to enable patients and providers secure authentication access to their health data. As described by the specification itself, SMART on FHIR:
...
### Clinician User Access[​](#clinician-user-access "Direct link to Clinician User Access")
...
Our [**TypeScript SDK**](/docs/sdk/core.medplumclient) provides a powerful, type-safe library to create and store healthcare data in Medplum. The SDK can be used in standalone applications and is used by [bots](/docs/bots).

## Prebuilt UI[​](#prebuilt-ui "Direct link to Prebuilt UI")

If you're building a [**React**](https://reactjs.org/) app with Medplum, our [**React Components**](/docs/api/react) can give you building blocks to get you started.

[Edit this page](https://github.com/medplum/medplum/blob/main/packages/docs/docs/api/index.md)
