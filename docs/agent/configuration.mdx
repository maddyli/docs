...
description: "This guide covers all local configuration options for the Medplum Agent, including how to configure the agent via command line arguments and the  file."
---
...
```
docker run -e MEDPLUM_LOG_LEVEL="DEBUG" \  -e MEDPLUM_BASE_URL="https://api.medplum.com" \  -e MEDPLUM_CLIENT_ID="my-client-id" \  -e MEDPLUM_CLIENT_SECRET="my-client-secret" \  -e MEDPLUM_AGENT_ID="123e4567-e89b-12d3-a456-426614174000" \  medplum/medplum-agent:latest
```
...
### Properties File Configuration[​](#properties-file-configuration "Direct link to Properties File Configuration")
...
```
baseUrl=https://api.medplum.comclientId=my-client-idclientSecret=my-client-secretagentId=123e4567-e89b-12d3-a456-426614174000
```

#### Setting Global Log Level[​](#setting-global-log-level "Direct link to Setting Global Log Level")
...
#### Applying Configuration Changes[​](#applying-configuration-changes "Direct link to Applying Configuration Changes")
...
## Logger Configuration[​](#logger-configuration "Direct link to Logger Configuration")
...
### Main Logger vs. Channel Logger[​](#main-logger-vs-channel-logger "Direct link to Main Logger vs. Channel Logger")

#### Main Logger[​](#main-logger "Direct link to Main Logger")
...
#### Channel Logger[​](#channel-logger "Direct link to Channel Logger")
...
### Why Separate Loggers?[​](#why-separate-loggers "Direct link to Why Separate Loggers?")
...
When troubleshooting connection issues or agent configuration problems, administrators can fetch main logger entries to diagnose the issue via the [Fetch Logs FHIR operation](/docs/agent/fetch-logs), without accessing any patient data.
...
### Logger Configuration Properties[​](#logger-configuration-properties "Direct link to Logger Configuration Properties")
...
#### Example 1: Different Log Levels[​](#example-1-different-log-levels "Direct link to Example 1: Different Log Levels")

Set the main logger to `INFO` and increase the channel logger to `DEBUG` for detailed message debugging:

```
logger.main.logLevel=INFOlogger.channel.logLevel=DEBUG
```

#### Example 2: Separate Log Directories[​](#example-2-separate-log-directories "Direct link to Example 2: Separate Log Directories")
...
```
logger.main.logDir=/var/log/medplum-agentlogger.channel.logDir=/var/log/medplum-agent/channels
```

**Windows:**

```
logger.main.logDir=C:\Logs\MedplumAgentlogger.channel.logDir=C:\Logs\MedplumAgent\Channels
```

#### Example 3: Different Retention Policies[​](#example-3-different-retention-policies "Direct link to Example 3: Different Retention Policies")

Keep more channel logs for compliance, but limit main logs to conserve space:

```
logger.main.maxFileSizeMb=10logger.main.filesToKeep=5logger.channel.maxFileSizeMb=50logger.channel.filesToKeep=30
```

#### Example 4: Production Configuration[​](#example-4-production-configuration "Direct link to Example 4: Production Configuration")

A production configuration that separates PHI-containing logs to a separate directory:

```
baseUrl=https://api.medplum.comclientId=production-client-idclientSecret=production-client-secretagentId=production-agent-id# Main logger: INFO level, moderate retentionlogger.main.logLevel=INFOlogger.main.logDir=/var/log/medplum/mainlogger.main.maxFileSizeMb=10logger.main.filesToKeep=10# Channel logger: WARN level (only warnings and errors), extended retentionlogger.channel.logLevel=INFOlogger.channel.logDir=/var/log/medplum/channelslogger.channel.maxFileSizeMb=20logger.channel.filesToKeep=60
```

### Log File Names[​](#log-file-names "Direct link to Log File Names")
...
```
medplum-agent-main-2024-10-02.logmedplum-agent-main-2024-10-03.logmedplum-agent-channels-2024-10-02.logmedplum-agent-channels-2024-10-03.log
```

### Verifying Logger Configuration[​](#verifying-logger-configuration "Direct link to Verifying Logger Configuration")
...
### Resetting to Defaults[​](#resetting-to-defaults "Direct link to Resetting to Defaults")
...
### Operational Recommendations[​](#operational-recommendations "Direct link to Operational Recommendations")
...
[Edit this page](https://github.com/medplum/medplum/blob/main/packages/docs/docs/agent/configuration.md)
